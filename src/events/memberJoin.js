...
module.exports = {
    ...
    once: false, // Indicates that the event should not be executed only once
    execute(member) {
        if (isTesting) return;
        if (member.guild.id !== guildId) return;

        const welcomeChannel = member.guild.channels.cache.get(welcomeChannelId); // Get the welcome channel
        const chatChannel = member.guild.channels.cache.get(chatChannelId); // Get the chat channel

        const welcomeEmbed = new EmbedBuilder() // Create a new embed message
            .setColor('#FFF7DC')
            .setTitle('🎉歡迎光臨')
        ...
        welcomeChannel.send({ content: `<@${member.id}>`, embeds: [welcomeEmbed] }); // Send the welcome message to the welcome channel
        chatChannel.send(`<@${member.id}> 向著星辰與深淵 歡迎來到原神夏洛特亞洲討論區`); // Send a welcome message to the chat channel
    }
}